...
(function () {
  ...
  /* ---------- render dos posts do Instagram ---------- */
  var grid = document.getElementById("instagramGrid");

  function fallbackHTML(post) {
    var link = post.url || (typeof INSTAGRAM_PROFILE !== "undefined" ? INSTAGRAM_PROFILE : "#");
    return (
      '<div class="ig-fallback">' +
        '<div class="ig-fallback-media ' + post.tone + '">' +
          '<div class="grain-layer" aria-hidden="true"></div>' +
          '<span class="ig-fallback-badge" aria-hidden="true">' +
          ...
          post.icon +
        '</div>' +
        '<div class="ig-fallback-body">' +
          '<p>' + post.legenda + '</p>' +
          '<a href="' + link + '" target="_blank" rel="noopener">Ver no Instagram →</a>' +
        '</div>' +
      '</div>'
    );
  }
  ...
  function embedHTML(post) {
    // formato oficial de incorporação do Instagram (blockquote + embed.js)
    return (
      '<div class="ig-embed">' +
        '<blockquote class="instagram-media" data-instgrm-permalink="' + post.url + '" data-instgrm-version="14" style="width:100%;"></blockquote>' +
      '</div>'
    );
  }

  function carregarEmbedScript(callback) {
  ...
  if (grid && typeof INSTAGRAM_POSTS !== "undefined") {
    var temFeedAoVivo = typeof SNAPWIDGET_EMBED !== "undefined" && SNAPWIDGET_EMBED.trim().length > 0;

    if (temFeedAoVivo) {
      // feed automático real, via SnapWidget (ou serviço equivalente)
      grid.classList.add("instagram-grid--live");
      grid.innerHTML = SNAPWIDGET_EMBED;
      return;
    }

    var temPostReal = INSTAGRAM_POSTS.some(function (p) { return !!p.url; });

    grid.innerHTML = INSTAGRAM_POSTS.map(function (post) {
      return post.url ? embedHTML(post) : fallbackHTML(post);
    }).join("");

    if (temPostReal) {
      carregarEmbedScript(function () {
